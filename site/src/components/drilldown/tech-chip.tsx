      ...
      {expanded && detail && (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--ink-4)" }}>Technology</span>
            <button onClick={() => setExpanded(false)} style={{ background: "none", border: "none", color: "var(--ink-4)", cursor: "pointer" }}>×</button>
          </div>
          <h3 style={{ fontSize: 16, fontWeight: 600, margin: "0 0 6px", color: "var(--ink)" }}>{detail.name}</h3>
          <p style={{ fontSize: 13, color: "var(--ink-2)", margin: "0 0 10px", lineHeight: 1.4 }}>{detail.blurb}</p>

          {detail.pros.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--c5)" }}>PROS</span>
              <ul style={{ margin: "4px 0 0", paddingLeft: 14, fontSize: 13, color: "var(--ink-2)" }}>
          ...
          {detail.cons.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--signal)" }}>CONS</span>
              <ul style={{ margin: "4px 0 0", paddingLeft: 14, fontSize: 13, color: "var(--ink-2)" }}>
