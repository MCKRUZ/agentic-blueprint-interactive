    ...
    <div className="flex flex-col h-full overflow-y-auto" style={{ padding: "16px 24px" }}>
      ...
      <div className="flex gap-6" style={{ minHeight: 460 }}>
        ...
        <div className="flex flex-col gap-2" style={{ width: 200, flexShrink: 0 }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 12, letterSpacing: "0.2em", color: "var(--ink-4)", marginBottom: 4 }}>
            TOPOLOGIES
          </div>
          {PATTERNS.map((p) => (
            <button
            ...
            >
              ...
              <div style={{ fontSize: 13, color: "var(--ink-4)", marginTop: 2 }}>{p.summary}</div>
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-3" style={{ width: 200, flexShrink: 0 }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 12, letterSpacing: "0.2em", color: "var(--ink-4)" }}>
            STATS
          </div>
          {active.stats && Object.entries(active.stats).map(([k, v]) => (
            <div key={k}>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--ink-4)", textTransform: "uppercase" }}>{k}</div>
              <div style={{ fontSize: 16, color: "var(--ink-2)" }}>{v}</div>
            </div>
          ))}
          {active.inTheWild.length > 0 && (
            <div>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--ink-4)", textTransform: "uppercase", marginBottom: 4 }}>
                IN THE WILD
              </div>
              {active.inTheWild.map((w, i) => (
                <div key={i} style={{ fontSize: 13, color: "var(--ink-3)", marginBottom: 2 }}>{w}</div>
              ))}
            </div>
          )}
        </div>
      ...
      <div style={{ maxWidth: 800, margin: "32px auto 0" }}>
        ...
        {active.stack.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <h3 style={{ fontFamily: "var(--mono)", fontSize: 13, letterSpacing: "0.15em", color: "var(--ink-3)", marginBottom: 8 }}>
              RECOMMENDED STACK
            </h3>
...
function ContentSection({ title, content }: { title: string; content: string }) {
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <p style={{ fontSize: 16, color: "var(--ink-2)", lineHeight: 1.65, whiteSpace: "pre-line" }}>{content}</p>
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
        {items.map((item, i) => (
          <li key={i} style={{ display: "flex", gap: 8, fontSize: 16, color: "var(--ink-2)", lineHeight: 1.65, marginBottom: 6 }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 12, color: "var(--ink-4)", flexShrink: 0, marginTop: 2 }}>
